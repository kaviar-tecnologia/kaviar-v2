...
router.get('/rides-report', async (req: Request, res: Response) => {
  try {
    ...
    if (search) {
      conditions.push(`(d.full_name ILIKE $${paramIdx} OR p.first_name ILIKE $${paramIdx} OR r.id::text ILIKE $${paramIdx})`);
      params.push(`%${search}%`);
      paramIdx++;
    }
    ...
    const dataSQL = `
      SELECT 
        ...
        r.driver_id, d.full_name as driver_name, p.first_name as passenger_first_name,
  ...
  try {
    ...
    const sql = `
      SELECT 
        ...
        d.full_name as driver_name, p.first_name as passenger_first_name,
        s.id IS NOT NULL as has_settlement, s.settled_at,
