...
describe('accountantApi configuration', () => {
  it('has withCredentials enabled', async () => {
    const { default: accountantApi } = await import('../src/services/accountantApi');
  ...
  it('does not use localStorage for tokens', () => {
    const code = readFileSync(resolve(__dirname, '../src/services/accountantApi.js'), 'utf8');
    expect(code).not.toContain('localStorage');
    expect(code).not.toContain('sessionStorage');
  });
  
  it('sets and clears access token in memory only', async () => {
    const { setAccountantAccessToken, clearAccountantAccessToken } = await import('../src/services/accountantApi');
